 ...
 * @author krawler
 */
public class accDepreciationImpl implements accDepreciationDAO {
    ...
    public KwlReturnObject updateDepreciationDetail(HashMap<String, Object> ddMap) throws ServiceException {
        ...
        try {
            ...
            DepreciationDetail dd = (DepreciationDetail) hibernateTemplate.get(DepreciationDetail.class, id);
            if (dd != null) {
                dd = buildDepreciation(dd, ddMap);
                hibernateTemplate.saveOrUpdate(dd);
            }
            list.add(dd);
        } catch (Exception e) {
    ...
    public DepreciationDetail buildDepreciation(DepreciationDetail dd, HashMap<String, Object> ddMap) {
        ...
        if (ddMap.containsKey("accountid")) {
            Account account = ddMap.get("accountid")==null?null:(Account)hibernateTemplate.get(Account.class, (String) ddMap.get("accountid"));
            dd.setAccount(account);
        }
        if (ddMap.containsKey("jeid")) {
            JournalEntry je = ddMap.get("jeid")==null?null:(JournalEntry)hibernateTemplate.get(JournalEntry.class, (String) ddMap.get("jeid"));
            dd.setJournalEntry(je);
        }
        if (ddMap.containsKey("companyid")) {
            Company company = ddMap.get("companyid")==null?null:(Company)hibernateTemplate.get(Company.class, (String) ddMap.get("companyid"));
...
//        query="from DepreciationDetail where period=? and account.ID=? and company.companyID=?";
	...
	public KwlReturnObject deleteDepreciationJE(String je) throws ServiceException {
        ...
        int count = HibernateUtil.executeUpdate(hibernateTemplate, selQuery, new Object[]{je});
    ...
    public KwlReturnObject addAssetDetail(HashMap<String, Object> assetMap) throws ServiceException {
        ...
        try {
            ...
            Asset ass = (Asset) hibernateTemplate.get(Asset.class, id);
            if (ass != null) {
            	ass = buildAsset(ass, assetMap);
                hibernateTemplate.saveOrUpdate(ass);
            }else{
            	ass = buildAsset(new Asset(), assetMap);
                hibernateTemplate.save(ass);
            }
            list.add(ass);
        } catch (Exception e) {
    ...
    public Asset buildAsset(Asset ass, HashMap<String, Object> assetMap) {
        if (assetMap.containsKey("id")) {
            Account account = assetMap.get("id")==null?null:(Account)hibernateTemplate.get(Account.class, (String) assetMap.get("id"));
            ass.setAccount(account);
        }
        if (assetMap.containsKey("purchaseJe")) {
            JournalEntry je = assetMap.get("purchaseJe")==null?null:(JournalEntry)hibernateTemplate.get(JournalEntry.class, (String) assetMap.get("purchaseJe"));
            ass.setPurchaseJe(je);
        }
        if (assetMap.containsKey("deleteJe")) {
            JournalEntry je = assetMap.get("deleteJe")==null?null:(JournalEntry)hibernateTemplate.get(JournalEntry.class, (String) assetMap.get("deleteJe"));
            ass.setDeleteJe(je);
        }
        if (assetMap.containsKey("companyid")) {
            Company company = assetMap.get("companyid")==null?null:(Company)hibernateTemplate.get(Company.class, (String) assetMap.get("companyid"));
