 ...
 * @author krawler
 */
public class AccCostCenterControllerTest  extends AbstractTransactionalDataSourceSpringContextTests {
    ...
    public AccCostCenterControllerTest(String testName) {
        super(testName);
    }
    ...
    /**
     * Test of getCostCenter method, of class accCostCenterController.
     */
    public void testGetCostCenter() {
        try {
            ...
            AccCostCenterController instance = new AccCostCenterController();
            instance.setaccCostCenterDAO(accCostCenterObj);
            instance.setTxnManager((HibernateTransactionManager) transactionManager);
            ModelAndView result = instance.getCostCenter(request, response);
    ...
    /**
     * Test of saveCostCenter method, of class accCostCenterController.
     */
    public void testSaveCostCenter_HttpServletRequest_HttpServletResponse() {
        try{
            System.out.println("saveCostCenter");
            MockHttpServletRequest request = new MockHttpServletRequest();
            MockHttpSession session = createUserMockSession(new JSONObject(sessionParams));
            request.setSession(session);
            request.setParameter("data", "[{id:\"\" ,ccid:\"LKSOFWE9\",name:\"ABGFJRIUWE\",description:\"Operations%20Department\",modified:true}]");
            request.setParameter("deleteddata", "[]");
            MockHttpServletResponse response = new MockHttpServletResponse();
            AccCostCenterController instance = new AccCostCenterController();
            instance.setaccCostCenterDAO(accCostCenterObj);
            instance.setTxnManager((HibernateTransactionManager) transactionManager);
            ModelAndView result = instance.saveCostCenter(request, response);
            JSONObject job = new JSONObject(result.getModel().get("model").toString());
            assertTrue(job.getBoolean("success"));
        } catch (ServiceException ex) {
            throw new AssertException("Service Exception - error creating user session");
