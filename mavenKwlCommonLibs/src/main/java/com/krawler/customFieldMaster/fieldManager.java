...
public class fieldManager {
    public static JSONObject getCustomColumnJSON(HttpServletRequest request, JSONObject tmpObj, String modulName, String recId, String moduleId) throws ServiceException, JSONException, SessionExpiredException {
        Session session = null;
        Transaction tx = null;
        try {
            session = HibernateUtil.getCurrentSession();
            tx = session.beginTransaction();
            ArrayList list = fieldManager.getFieldData(session, modulName, recId);
            ArrayList list1 = fieldManager.getOnlyFieldName(session, request, moduleId);
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (StringUtil.equal(item1[j].toString(), item[0].toString())) {
                        if ((StringUtil.equal(item[3].toString(), "3")) && !(StringUtil.equal(item[1].toString(), "undefined")) && !(StringUtil.isNullOrEmpty(item[1].toString()))) {
                            ...
                            tmpObj.put(item[0].toString(), fieldManager.customComboValue(session, item[1].toString()));
                        } else {
    ...
    public static ArrayList getFieldData(Session session,String module,String moduleRecId){
    ...
    public static ArrayList getFieldName(Session session,String module,String moduleRecId){
        ArrayList returnList = new ArrayList();
    ...
    public static ArrayList getOnlyFieldName(Session session, HttpServletRequest request, String moduleId) throws SessionExpiredException, ServiceException {
        ArrayList returnList = new ArrayList();
        try {
    ...
    public static String customComboValue(Session session,String df) {
        String status = "";
        if (df != null) {
            ArrayList returnList = new ArrayList();
    ...
    public static JSONObject applyColumnFormulae(HttpServletRequest request, JSONObject tmpObj, String moduleid, String modName) throws ServiceException, JSONException {
        String fieldname = "";
        Session session = null;
        try {
            session = HibernateUtil.getCurrentSession();
    ...
    public static boolean storeCustomFields(JSONArray jarray,String modulename,boolean  isNew,String modulerecid){
        Session session = null;
        Transaction tx = null;
        try {
            session = HibernateUtil.getCurrentSession();
    ...
    // for converted lead
    public static boolean storeCustomFields(JSONArray jarray, String modulename, boolean isNew, String modulerecid, ArrayList ll) {
        Session session = null;
        Transaction tx = null;
        try {
            session = HibernateUtil.getCurrentSession();
...
ENGINE = InnoDB;
*/
    public static String createNewField(String fieldlabel,int isessential,int maxlength, int validationtype,int fieldtype,int moduleid,String customregex,String companyid,String combodata, String formulae, String iseditable, HttpServletRequest request){
        int result=0;
        String msg = "";
        Session session=null;
        try{
  ...
  tx.commit();
        ...
        if(!StringUtil.isNullOrEmpty(formulae)) {
            ...
            msg = "Column with the same name already exsits.";
        } catch(Exception e){
            msg = "Custom Column cannot be created.";
        }finally{
            HibernateUtil.closeSession(session);
        }

        if(result == 1){
            msg = "New column created successfully.<br/> Please close the tab and open again to use the new field";
            return msg;
        }else{
...
public static ArrayList getColumnModel(String moduleid,String companyid){
...
public static String setCustomColumnFormulae(Session session, HttpServletRequest request) throws Exception {
...
ENGINE = InnoDB;*/
public static ArrayList getComboData(String fieldid,String companyid){
    ArrayList colmodel = new ArrayList();
    Session session = HibernateUtil.getCurrentSession();
    try{
...
public static String preferenceDate(Session session, HttpServletRequest request, Date date, int timeflag) throws SessionExpiredException {
