...
public class XLSDataExtractor extends HttpServlet {
    ...
    public JSONObject parseXLS(String filename, int sheetNo) throws FileNotFoundException, IOException, JSONException{
            ...
            for(int i=0;i <= noOfRowsDisplayforSample;i++) {
                ...
                if(row==null){jArr.put(obj);continue;}
                if(maxCol<row.getLastCellNum())maxCol=row.getLastCellNum();
                for(int j=0; j<row.getLastCellNum();j++){
                    ...
                    switch(cell.getCellType()){
                        case Cell.CELL_TYPE_NUMERIC: val=Double.toString(cell.getNumericCellValue()); break;
                        case Cell.CELL_TYPE_STRING: val=cell.getRichStringCellValue().getString(); break;
                    }
