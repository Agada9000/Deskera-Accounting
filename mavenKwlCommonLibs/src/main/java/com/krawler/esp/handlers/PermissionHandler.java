...
public class PermissionHandler {
    ...
    public static JSONObject getPermissionCode(Session session, HttpServletRequest request)
        ...
        try {
            ...
            String query="select feature.featureID, permissionCode from UserPermission up where roleId.userId.userID=? and roleId.roleId.roleid=?";
            List list = HibernateUtil.executeQuery(session, query, new Object[]{userid, roleid});
            Iterator ite = list.iterator();
            JSONArray jArr=new JSONArray();
            while(ite.hasNext() ) {
                Object[] row = (Object[])ite.next();
    ...
    public static JSONObject getPermissionsValidate(Session session, String userid, HttpServletRequest request)
        ...
        try {
            if (!isSuperAdmin(request)) {
                ...
                query = "select feature.featureName, permissionCode from UserPermission up where roleId.userId.userID=?";
                list = HibernateUtil.executeQuery(session, query, userid);
                ite = list.iterator();
                while (ite.hasNext()) {
                    Object[] row = (Object[]) ite.next();
    ...
    public static void saveRole(Session session, HttpServletRequest request) throws ServiceException {
        try {
            ...
            role.setCompany((Company) session.get(Company.class, AuthHandler.getCompanyid(request)));
