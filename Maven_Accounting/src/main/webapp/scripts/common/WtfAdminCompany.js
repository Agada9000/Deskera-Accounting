...
//        }
            ...
            if(day) {
                if(this.searchInHolilist(day.format('Y-m-d'))) {
                    ...
                    Wtf.getCmp("holidayDateField").reset();
                     ...
                     WtfComMsgBox(["Alert", "Date '"+day.format('Y-m-d')+"' is already marked as a holiday."],2);
            } else
                WtfComMsgBox(["Alert","Please give a valid date"],2);
        } else
            WtfComMsgBox(["Alert","Please give required data"],2);
    },
    ...
    genSuccessResponse:function(response){
        if(response.success==true)
            updatePreferences();
        WtfComMsgBox(['Company Management',response.msg],response.success*2+1);
    },
