...
Wtf.extend(Wtf.selectTempWin,Wtf.Window,{
    ...
    exportTemplate:function(namePanel){
        ...
        if(smTmp.getCount()<1){
            ...
            var url = exportUrl+"?"+this.mode+this.extra+"&config="+configstr+"&filename="+this.filename+"&filetype="+ this.type+"&stdate="+this.stdate+"&enddate="+this.enddate+"&accountid="+this.accountid
                     +"&get="+this.get+"&gridconfig="+encodeURIComponent(this.gridConfig)+this.paramstring+"&deleted="+this.deleted+"&nondeleted="+this.nondeleted;
