...
Wtf.extend(Wtf.customReport,Wtf.Panel,{
    ...
    Addsubtitle:function(){
        ...
        this.hfieldset.insert(this.count,new Wtf.Panel({
            ...
            border: false,
            html:'<a href=\"#\" class ="attachmentlink" style ="margin-left:5px" onclick=\"removesubtitle(\''+pid+'\',\''+this.id+'\')\">Remove</a>',
            items:textfield
            })
        );
        this.hfieldset.setHeight(this.attachheight);
        document.getElementById('subtitlelink'+this.id).innerHTML = "Add another subtitle";
        if(this.count>5)
                ...
                buttons:[{
                    ...
                    handler: function() {
                        if(!nameField.isValid()) {
                            WtfComMsgBox([WtfGlobal.getLocaleText("acc.common.alert"), "Could not save template. Please try again."],1);
                            return;
                        }
...
//            url: Wtf.req.base + 'template.jsp',
        ...
        function() {
            WtfComMsgBox(["Error", "Could not save template. Please try again."],1);
        });
    },
    saveReportTemplate:function(win, nameField, descField){
        ...
        if(tname == null && tname == "") {
            Wtf.MessageBox.alert("Error", "The report template has not been saved. Please check entries and try again.");
            return;
        }
        Wtf.Ajax.requestEx({
//            url: Wtf.req.base + 'template.jsp',
        ...
        function() {
            WtfComMsgBox(["Error", "Could not create template. Please try again."],1);
        });
        win.close();
    },
    exportPdf:function() {
