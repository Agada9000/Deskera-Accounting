        ...
        if (com.krawler.esp.handlers.SessionHandler.isValidSession(request, response)) {
            ...
            try {
              //  session = HibernateUtil.getCurrentSession();
                ...
                ByteArrayOutputStream baos=createPdf(currencyid, billid, formatter, mode,amount,getServletContext().getRealPath(""),logoPath);
...
//                HibernateUtil.closeSession(session);
    ...
    public ByteArrayOutputStream createPdf(String currencyid, String billid, DateFormat formatter, int mode, double amount, String contextpath, String logoPath) throws JSONException {
...
//            GoodsReceipt gRec = (GoodsReceipt) session.get(GoodsReceipt.class, billid);
                ...
                String[] values2 = {gRec.getVendorEntry().getAccount().getName(),authHandlerDAOObj.getFormattedCurrency(amount,currencyid),gRec.getMemo()};
...
//                    vendor=(Vendor)session.get(Vendor.class,acc.getID());
                ...
                String[] values2 = {vendor.getName(),authHandlerDAOObj.getFormattedCurrency(amount,currencyid),deb.getMemo()};
...
//                    customer=(Customer)session.get(Customer.class,acc.getID());
                ...
                String[] values2 = {customer.getName(),authHandlerDAOObj.getFormattedCurrency(amount,currencyid),cre.getMemo()};
                String[] values3 = new String[3];
                String[] values4 = new String[0];
...
//                PurchaseOrder po = (PurchaseOrder) session.get(PurchaseOrder.class, billid);
                ...
                String[] values2 = {po.getVendor().getName(),authHandlerDAOObj.getFormattedCurrency(amount,currencyid),po.getMemo()};
                String[] values3 = new String[5];
...
//                SalesOrder so = (SalesOrder) session.get(SalesOrder.class, billid);
                ...
                String[] values2 = {so.getCustomer().getName(),authHandlerDAOObj.getFormattedCurrency(amount,currencyid),so.getMemo()};
                String[] values3 = new String[5];
...
//                    vendor=(Vendor)session.get(Vendor.class,acc.getID());
                ...
                String[] values2 = {vendor.getName(),authHandlerDAOObj.getFormattedCurrency(amount,currencyid),pay.getMemo()};
                ...
                String[] values2 = {quotation.getCustomer().getName(),authHandlerDAOObj.getFormattedCurrency(amount,currencyid),quotation.getMemo()};
                String[] values3 = new String[5];
    ...
    private  PdfPTable createDebitTable(String header[], String values[], String currencyid, DebitNote deb) throws DocumentException, ServiceException, JSONException {
    ...
    private PdfPTable createCreditTable(String header[], String values[], String currencyid, CreditNote cre) throws DocumentException, ServiceException, JSONException {
        PdfPTable table = new PdfPTable(header.length);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{30, 30, 40});
        PdfPCell cell = null;
    ...
    private PdfPTable createDebitTable(String header[], String values[], String currencyid, BillingDebitNote deb) throws DocumentException, ServiceException, JSONException {
        PdfPTable table = new PdfPTable(header.length);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{30, 30, 40});
        PdfPCell cell = null;
    ...
    private PdfPTable createCreditTable(String header[], String values[], String currencyid, BillingCreditNote cre) throws DocumentException, ServiceException, JSONException {
        PdfPTable table = new PdfPTable(header.length);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{30, 30, 40});
        PdfPCell cell = null;
    ...
    private PdfPTable createGoodsReceiptTable(String[] header, String[] values, String currencyid, GoodsReceipt gr) throws DocumentException {
        double total = 0;
        PdfPTable table = new PdfPTable(header.length);
        table.setWidthPercentage(100);
        ...
        for (int i = 0; i < header.length; i++) {
            ...
            while (itr.hasNext()) {
                ...
                cell.setBorder(Rectangle.LEFT + Rectangle.RIGHT);
                table.addCell(cell);
                cell = new PdfPCell(new Paragraph(authHandlerDAOObj.getFormattedCurrency(row.getRate(), currencyid), fontSmallRegular));
                cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
                cell.setBorder(Rectangle.LEFT + Rectangle.RIGHT);
                table.addCell(cell);
                ...
                total += amount;
                cell = new PdfPCell(new Paragraph(authHandlerDAOObj.getFormattedCurrency(amount, currencyid), fontSmallRegular));
                cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
                cell.setBorder(Rectangle.LEFT + Rectangle.RIGHT);
                table.addCell(cell);
            }
        ...
        cell.setHorizontalAlignment(Element.ALIGN_LEFT);
        table.addCell(cell);
        cell = new PdfPCell(new Paragraph(authHandlerDAOObj.getFormattedCurrency(total, currencyid), fontTblMediumBold));
        cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
        cell.setPadding(4);
        table.addCell(cell);
        ...
        cell.setHorizontalAlignment(Element.ALIGN_LEFT);
        table.addCell(cell);
        cell = new PdfPCell(new Paragraph(authHandlerDAOObj.getFormattedCurrency(gr.getVendorEntry().getAmount(),currencyid) ,fontTblMediumBold));
        cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
        table.addCell(cell);
        return table;
    }

    private PdfPTable createPurchaseOrderTable(String[] header, String[] values, String currencyid, PurchaseOrder po) throws  DocumentException {
        double total = 0;
        PdfPTable table = new PdfPTable(header.length);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{30, 15, 15, 20, 20});
    ...
    private PdfPTable createSalesOrderTable(String[] header, String[] values, String currencyid, SalesOrder so) throws DocumentException {
        double total = 0;
        PdfPTable table = new PdfPTable(header.length);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{30, 15, 15, 20, 20});
    ...
    private PdfPTable createPurchaseOrderTable(String[] header, String[] values, String currencyid, BillingPurchaseOrder po) throws DocumentException {
        double total = 0;
        PdfPTable table = new PdfPTable(header.length);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{30, 15, 15, 20, 20});
    ...
    private PdfPTable createSalesOrderTable(String[] header, String[] values, String currencyid, BillingSalesOrder so) throws DocumentException {
        double total = 0;
        PdfPTable table = new PdfPTable(header.length);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{30, 15, 15, 20, 20});
    ...
    private PdfPTable createPaymentTable(String[] header, String[] values, String currencyid, Payment pay, DateFormat formatter) throws DocumentException {
        double total = 0;
        PdfPTable table = new PdfPTable(header.length);
        table.setWidthPercentage(100);
        ...
        while (itr.hasNext()) {
            ...
            cell.setBorder(Rectangle.LEFT + Rectangle.RIGHT);
            table.addCell(cell);
            cell = new PdfPCell(new Paragraph(authHandlerDAOObj.getFormattedCurrency(row.getAmount(), currencyid), fontSmallRegular));
            cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
            cell.setBorder(Rectangle.LEFT + Rectangle.RIGHT);
            table.addCell(cell);
    ...
    private PdfPTable createQuotationTable(String[] header, String[] values, String currencyid, Quotation quotation) throws DocumentException {
        double total = 0;
        PdfPTable table = new PdfPTable(header.length);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{30, 15, 15, 20, 20});
